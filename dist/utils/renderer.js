...
// Updated for React - no longer dependent on Lit component
const cheddar = {
    // Core functionality
    initializeGemini,
    startCapture,
    ...
    // Status and response functions - now handled by React
    // These are kept for compatibility but will be handled via IPC
    setStatus: text => {
        console.log('[cheddar.setStatus]', text);
        // Status updates are handled by React via IPC events
    },
    setResponse: response => {
        console.log('[cheddar.setResponse]', response);
        // Response updates are handled by React via IPC events
    },
    
    // App state functions - now use localStorage since React manages state
    getCurrentView: () => localStorage.getItem('currentView') || 'main',
